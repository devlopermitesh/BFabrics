...
export const useSnapImages=()=>useQuery({
    queryKey:['snapImages'],
    queryFn:view.GetSnapImages,
    retry: 2,
})

export const useStatics=()=>useQuery({
    queryKey:['statics'],
    queryFn:view.GetCounterStattics,
    retry: 2,
})

export const useHighFeatures=()=>useQuery({
    queryKey:['Features'],
    queryFn:view.GetHighlightFeature,
    retry: 2,
})

export const useWorkFlow=()=>useQuery({
    queryKey:["workflow"],
...
export const useTestimonal=()=>useQuery({
    queryKey:["Testimonal"],
    queryFn:view.GetTestinomal
})

export const useFAQ=(category: string) =>useQuery({
queryKey:['Faq',category],
queryFn:() =>view.GetFAQ(category)
})
export const newAddition=()=>useQuery({
    queryKey:["Addition"],
