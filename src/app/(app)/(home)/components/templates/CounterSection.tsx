     ...
     {renderWithStates(
        (data) => (
            ...
            <div className="flex flex-row w-full justify-around md:justify-start gap-3">
                {data[0].category.map((item)=><Link key={item} href={`/category/${item}`} className="p-2 px-4 border border-dashed border-inactive rounded text-inactive font-thin font-primary">{item}</Link>)}
            </div>
            <div className="flex flex-col w-ful py-3">
            <p className="font-primary font-semibold uppercase text-white text-2xl md:text-3xl ">{data[0].punchLine}</p>
           ...
           {data[0].descrption}
            ...
            <div className="grid grid-cols-2 grid-rows-2 gap-3 md:gap-0 mt-4 flex-1">
                ...
                <StaticsCount key={item.name} name={item.name} count={item.count}/>
                    ))
                }
            </div>
